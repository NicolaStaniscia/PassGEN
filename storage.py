...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        sign_up = st.user_registration(name, password)
        if sign_up:
        ...
        name = input("Username: ")
        password = input("Password: ")
        access = st.user_login(name, password)
        if access:
            print(f"Access granted to {name} (user id: {st.user_id})")
            ...
            if choice == "1":
                ...
                password = input("Password: ")
                storing = st.add_password(service, username, password)
                if storing:
